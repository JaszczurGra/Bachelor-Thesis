...
class STRRT_Planer(BasePathfinding):
    ...
    def propagate(self,state, control, result):
        result[0] = state[0] + state[2] * math.cos(control[0])  
        result[1] = state[1] + state[2] * math.sin(control[0])  
        result[2] = state[2] + control[0]
...
if __name__ == "__main__": 
    ...
    car_planner = STRRT_Planer(max_runtime=15)
