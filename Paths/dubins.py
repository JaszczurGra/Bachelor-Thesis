...
def main():
    ...
    Wptz = [Waypoint(np.random.random()*4000, np.random.random()*4000, np.random.random()*360) for _ in range(11)]
    # Run the code
    i = 0
    while i<len(Wptz)-1:

        # Calculate the dubins path between waypoints speed, max_psi
        param = calcDubinsPath(Wptz[i], Wptz[i+1], 90, 30)
