...
# --- New kinodynamic rewiring helpers ---
...
def kinodynamic_connect(q_from, q_to, min_turning_radius, step_size=0.5):
    """
    Try to produce a kinodynamic-feasible trajectory from q_from->q_to.
    - If dubins available, return sampled dubins path (Nx3).
    - Otherwise fallback to straight-line interpolation with heading interpolation.
    Returns trajectory (Nx3) or None if failed.
    """
...
def rewire_nearby(tree, new_idx, img, min_turning_radius, robot_radius, neighbor_radius=60, step_size_connect=0.5):
    """
    Attempt to rewire neighbors to use new_idx as parent when it reduces cost
    and a feasible kinodynamic connector exists.
    Updates tree in-place: parent, trajectories, path_costs for rewired nodes and their descendants.
    """
    n = len(tree['nodes'])
    ...
    for i in range(n):
        if i == new_idx:
            continue
        d = np.linalg.norm(np.array(tree['nodes'][i][:2]) - np.array(new_q[:2]))
        if d <= neighbor_radius:
            neighbors.append(i)

    for nb in neighbors:
        # try connect new -> neighbor (reparent neighbor to new)
        traj = kinodynamic_connect(new_q, tree['nodes'][nb], min_turning_radius, step_size=step_size_connect)
        if traj is None:
            continue
        # collision check
...
# --- Main planner with rewiring integrated ---
def kinodynamic_rrt(img, start, goal, max_iter, step_size, goal_radius,
    ...
    for it in range(max_iter):
        ...
        # Rewiring: attempt to reparent nearby nodes to produce shorter/straighter connections
        # neighbor_radius can be tuned; scale it with display scale if needed
        neighbor_radius = max(30, int(60 / max(1, scale)))
        rewire_nearby(tree, new_idx, img, min_turning_radius, robot_radius,
                      neighbor_radius=neighbor_radius, step_size_connect=1.0)
...
if __name__ == "__main__":
    ...
    try:
        img = load_img("Paths\\Pictures\\map_2_d.png")
    except Exception:
